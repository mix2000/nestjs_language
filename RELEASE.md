# Release Guide v0.0.10

## ✅ Релиз подготовлен!

### Что сделано:
- ✅ Версия обновлена до **0.0.10** в package.json
- ✅ **Критическое исправление**: Реализована ленивая загрузка зависимостей
...
npm install ozma-nestjs-language@0.0.10
```
...
### Критическое исправление v0.0.10:

**Проблема:** При использовании только одного ORM (например, Sequelize), модуль пытался загрузить зависимости другого ORM (TypeORM), что приводило к ошибке `Cannot find module '@nestjs/typeorm'`.

**Решение:** Реализована ленивая загрузка (lazy loading) через динамические `require()`. Теперь загружаются только те зависимости, которые реально используются.

```typescript
// Раньше - статические импорты загружались всегда
import { TypeOrmModule } from '@nestjs/typeorm';  // ❌ Ошибка если не установлен
import { SequelizeModule } from '@nestjs/sequelize';  // ❌ Ошибка если не установлен

// Теперь - динамическая загрузка по требованию
if (options.orm === 'typeorm') {
    const TypeOrmModule = require('@nestjs/typeorm').TypeOrmModule;  // ✅ Загружается только если нужен
}
```

### Возможности из предыдущих версий v0.0.8:
...
- Критическое исправление: Ленивая загрузка зависимостей
...
- ✅ Версия в package.json уникальна (0.0.10 еще не опубликована)
